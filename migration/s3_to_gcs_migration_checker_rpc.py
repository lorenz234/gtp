...
def get_date_from_rpc(w3, block_start):
    """Get the actual date for a block using RPC call."""
    try:
        timestamp = block_cache.get_block_timestamp(w3, block_start)
        if timestamp:
            date_str = datetime.fromtimestamp(timestamp).strftime('%Y-%m-%d')
            return date_str
        return None
    except Exception as e:
        print(f"Error getting date from RPC for block {block_start}: {e}")
        return None
...
def check_single_file_rpc(args):
    """Check a single S3 file for its GCS counterpart using RPC for exact timestamp."""
    file_info, s3_bucket, gcs_bucket, chain, processed_files, w3 = args
    ...
    try:
        ...
        # Get exact date from RPC
        if w3:
            date_str = get_date_from_rpc(w3, block_start)
            result['rpc_date'] = date_str
            
            if not date_str:
                result['error'] = 'Could not get block timestamp from RPC'
                return result
        else:
            result['error'] = 'No RPC connection available'
            return result
...
def check_migration_rpc(s3_bucket, gcs_bucket, folder_prefix, chain, max_sample_files=None, skip_tracking=False, delete_migrated=False, dry_run_delete=False, max_workers=10):
    """
    Checks migration status using RPC calls for exact block timestamps with parallel processing.
    ...
    """
    print(f"\n{'='*60}")
    print(f"RPC-BASED MIGRATION CHECKER (Exact Timestamps + Parallel Processing)")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_file = {
            executor.submit(check_single_file_rpc, (file_info, s3_bucket, gcs_bucket, chain, processed_files, w3)): file_info
        ...
        with tqdm(total=len(s3_files_info), desc="Checking files", unit="file") as pbar:
            for future in as_completed(future_to_file):
                try:
                    ...
                    # Count RPC calls (approximate)
                    if result.get('rpc_date'):
                        stats['rpc_calls'] += 1
    ...
    print(f"\n{'='*60}")
    print(f"RPC-BASED MIGRATION CHECK RESULTS")
    print(f"{'='*60}")
    ...
    print(f"RPC calls made: {stats['rpc_calls']} (cached: {block_cache.get_cache_size()})")
    ...
    if missing_files:
        ...
        for result in missing_files[:10]:
            ...
            print(f"  - {result['s3_file']} → {result['gcs_file']} (RPC date: {result.get('rpc_date')}){tracking_info}")
    ...
    report_file = f"migration_check_rpc_{chain}_{folder_prefix.replace('/', '_')}.json"
    try:
        with open(report_file, 'w') as f:
            json.dump({
                'timestamp': datetime.now().isoformat(),
                'method': 'rpc_exact_timestamps_parallel',
                'config': {
                    's3_bucket': s3_bucket,
...
def main():
    ...
    # Check arguments
    if len(sys.argv) < 2:
        print("Usage: python s3_to_gcs_migration_checker_rpc.py <command> [options]")
        print("\nCommands:")
        ...
        print("\nExamples:")
        print("  python s3_to_gcs_migration_checker_rpc.py check unichain/ unichain")
        print("  python s3_to_gcs_migration_checker_rpc.py check unichain/ unichain --workers 20")
        print("  python s3_to_gcs_migration_checker_rpc.py debug migration_check_rpc_unichain_unichain_.json")
        print("  python s3_to_gcs_migration_checker_rpc.py debug migration_check_rpc_unichain_unichain_.json 20")
        ...
        print("  - RPC caching: Block timestamp caching to reduce redundant RPC calls")
    ...
    if command == "debug":
        if len(sys.argv) < 3:
            ...
            print("Usage: python s3_to_gcs_migration_checker_rpc.py debug <report_json> [max_files]")
            sys.exit(1)
        ...
        if len(sys.argv) < 4:
            ...
            print("Usage: python s3_to_gcs_migration_checker_rpc.py check <s3_folder_prefix> <chain_name> [options]")
            sys.exit(1)
        ...
        print(f"Starting ENHANCED RPC-based migration check...")
        if delete_migrated:
        ...
        start_time = time.time()
        check_migration_rpc(s3_bucket, gcs_bucket, folder_prefix, chain, max_sample_files, skip_tracking, delete_migrated, dry_run_delete, max_workers)
        
        total_time = time.time() - start_time
        print(f"\nCheck completed in {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
        print(f"🎯 100% accurate timestamps via RPC calls (cached for efficiency)")
...
# python s3_to_gcs_migration_checker_rpc.py check unichain/ unichain --no-tracking
# python s3_to_gcs_migration_checker_rpc.py check loopring/ loopring --delete --workers 12
# python s3_to_gcs_migration_checker_rpc.py check unichain/ unichain --delete
# python s3_to_gcs_migration_checker_rpc.py check unichain/ unichain --dry-run-delete --workers 15

# Debug problematic files:
# python s3_to_gcs_migration_checker_rpc.py debug migration_check_rpc_unichain_unichain_.json
# python s3_to_gcs_migration_checker_rpc.py debug migration_check_rpc_unichain_unichain_.json 20
